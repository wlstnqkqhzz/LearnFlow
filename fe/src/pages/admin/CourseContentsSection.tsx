...
export function CourseContentsSection({ courseId }: { courseId: number }) {
  ...
  return <section className="surface detail-section course-content-section" aria-labelledby="course-contents-heading">
    <div className="section-heading"><div><h2 id="course-contents-heading">콘텐츠</h2><p className="admin-hint">위·아래로 배치한 뒤 전체 순서를 한 번에 저장합니다.</p></div><button className="admin-button primary-button" type="button" disabled={dirty} title={dirty ? '순서를 먼저 저장하거나 되돌려 주세요.' : undefined} onClick={() => setModal('create')}>+ 콘텐츠 추가</button></div>
