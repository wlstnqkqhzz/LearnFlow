...
export function ConfirmDialog({ title, description, label, onConfirm, onClose, pending, error, tone = 'danger' }: { title: string; description: string; label: string; onConfirm: () => void; onClose: () => void; pending: boolean; error: string; tone?: 'danger' | 'primary' }) {
  return <Modal title={title} onClose={onClose} busy={pending}><p className="confirm-description">{description}</p>{error && <p className="admin-error" role="alert">{error}</p>}<div className="admin-actions"><button type="button" className="admin-button" disabled={pending} onClick={onClose}>취소</button><button type="button" className={`admin-button ${tone === 'primary' ? 'primary-button' : 'danger-button'}`} disabled={pending} onClick={onConfirm}>{pending ? '처리 중…' : label}</button></div></Modal>
}
